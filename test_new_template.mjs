...
const templatePath = path.join(process.cwd(), 'public/templates/kwitansi_new.docx');
...
try {
  ...
  fs.writeFileSync('/tmp/test_new_output.docx', Buffer.from(report));
  console.log('SUCCESS: New template works with frontend data!');
} catch (error) {
